...
class DB {
    ...
    public function __construct($dsn, $user, $password) {
        ...
        echo '\n'.$password;
        echo '\n';
        $this->link = mysqli_connect($dsn, $user, $password);
    }

    public function checkMode($value = true) {
        $this->check = $value;
    }
    ...
    public function selectDb($name_db) {
        mysqli_select_db($this->link, $name_db) or super_die(array(mysqli_error($this->link), $sql, __METHOD__));
    }
    ...
    public function insert($table_name, $parameter) {
        ...
        foreach ($parameter as $key => $value ) {
            $sql_head .= sprintf("`%s`, ", $this->toSql($key));
            ...
            $sql_foot .= "NOW(), ";
                ...
                $sql_foot .= sprintf("'%s', ", $this->toSql($value));
        }
    ...
    public function update($table_name, $parameter, $condition = null, $limit = null) {
        ...
        foreach($parameter as $key => $value) {
            ...
            elseif($value == 'NOW()')
            $sql .= sprintf(" `%s` = NOW(),", $this->toSql($key));
            else
                $sql .= sprintf(" `%s` = '%s',", $this->toSql($key), $this->toSql($value));
        }
        $sql = substr($sql, 0, -1);
        if(!empty($condition)) {
            $sql .= " WHERE";
            foreach($condition as $key => $value) {
                $sql .= sprintf(" `%s` = '%s' AND", $this->toSql($key), $this->toSql($value));
            }
            $sql = substr($sql, 0, -3);
        }
